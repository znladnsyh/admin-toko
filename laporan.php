        ...
        <div class="card-body bg-white py-2 px-1 border-laporan">
            <div class="row mx-auto align-items-center">
                ...
                <div class="col-auto pl-0 pt-2">
                    ...
                    <h4 class="1"><?php 
                    $itungpeterjual = mysqli_query($conn, "SELECT SUM(quantity) as jumlahterjual FROM tb_nota");
                    $itungpeterjual1 = mysqli_fetch_assoc($itungpeterjual);
                    $itungpeterjual2 = $itungpeterjual1['jumlahterjual'];

                    if($itungpeterjual2 !== null && $itungpeterjual2 > 0){
    ...
    <div class="card-body">
        <table class="table table-striped table-sm table-bordered dt-responsive nowrap" id="table" width="100%">
            ...
            <tbody>
                <?php
                    $no=1;
                    $data_produk=mysqli_query($conn,"SELECT * FROM laporan l LEFT JOIN pelanggan e ON e.idpelanggan=l.idpelanggan ORDER BY idlaporan ASC");
                ...
                <tr>
                    ...
                    <td><?php echo $d['nama_pelanggan'] ? $d['nama_pelanggan'] : 'Pelanggan tidak dikenal' ?></td>
                    <td><?php 
...
<?php 
	if(!empty($_GET['hapus'])){
        ...
        if($hapus_data&&$hapus_data1){
            echo '<script>alert("Berhasil Hapus data laporan");window.location="laporan.php"</script>';
        } else {
            echo '<script>alert("gagal Hapus data laporan");history.go(-1);</script>';
        }
	}

    // Hapus semua data dari tabel laporan dan tb_nota
    if (isset($_POST['hapus_semua'])) {
        $hapus_laporan = mysqli_query($conn, "DELETE FROM laporan");
        $hapus_tb_nota = mysqli_query($conn, "DELETE FROM tb_nota");

        // Cek apakah query berhasil
        if ($hapus_laporan && $hapus_tb_nota) {
            echo '<script>alert("Berhasil menghapus semua data.");window.location="laporan.php"</script>';
        } else {
            echo '<script>alert("Gagal menghapus semua data.");history.go(-1);</script>';
        }
    }
?>
